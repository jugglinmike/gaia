...
var ValuePicker = (function() {
  ...
  VP.prototype.refresh = function() {
    empty(this.element);
    this.ui();
  };

  VP.prototype.ui = function() {
    var lower = this._lower;
    var upper = this._upper;
    var units = this._valueDisplayedText.length;
    var node;
    ...
    node = this._pickerUnits[0];

    // this._pickerUnitsHeight = node.clientHeight;
    this._pickerHeight = node.clientHeight * units;
  ...
  VP.prototype.addPickerUnit = function(index) {
    ...
    unit.textContent = this._valueDisplayedText[index];
