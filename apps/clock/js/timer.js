...
// TODO: Timers should be stored in mozAlarm?
//
// As soon as this script is loaded and execute,
// check for any existing timers.
asyncStorage.getItem('active_timer', function(result) {
  if (result !== null) {
    timer = new Timer(JSON.parse(result));
  }
});
 ...
 *
 */
Timer.Panel = function() {
  this.timer = timer;
  this.nodes = {};

  this.picker = window.p = new Picker({
    context: 'time-picker',
    ranges: {
      hour: [0, 24],
      minute: [0, 60],
  ...
  [
    ...
    'pause', 'start', 'sound', 'time', 'panel'
  ].forEach(function(id) {
    this.nodes[id] = document.getElementById('timer-' + id);
  }, this);
...
Timer.Panel.prototype = {
  constructor: Timer.Panel,
  /**
  ...
  handleEvent: function(event) {
    var meta = wm.get(event.target);
    var panel = meta.panel;
    var nodes = panel.nodes;
    var dialog = View.instance(nodes.dialog);
    ...
    if (panel.timer && panel.timer[meta.action]) {
      ...
      if (meta.action === 'create') {
        ...
        // Update the panel's timer property
        panel.timer = timer;

        // Bind the tick and end events to the
        // newly created timer object.
        timer.on('tick', panel.update.bind(panel)).start();
        timer.on('end', panel.dialog.bind(panel));
...
/**
 * Timer.init Create or reuse a Timer.Panel
 *
 * @return {Timer.Panel} Timer.Panel object.
 */
Timer.init = function() {
  return panel || (panel = new Timer.Panel());
};
